...
class Astropix3LBModel():
    ...
    async def enableLoopback(self,flush=True):
        ...
        regval =  await getattr(self.driver.rfg, f"read_lane_{self.layer}_cfg_ctrl")()
        regval |= (1<<5)
        await getattr(self.driver.rfg, f"write_lane_{self.layer}_cfg_ctrl")(regval,flush)
    ...
    async def writeMISOBytes(self,b:bytes,flush : bool =True):
        await getattr(self.driver.rfg, f"write_lane_{self.layer}_loopback_miso_bytes")(b,flush)
