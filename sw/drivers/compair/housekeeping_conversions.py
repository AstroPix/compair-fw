...
if __name__ == "__main__":
    for adc in range(3):
        for ch in range(8):
            print(f"{hk_converter_list[adc][ch][1]} = {hk_converter_list[adc][ch][3](1240)} = {convert(adc,ch,1240)}")

    for adc in range(3):
        for ch in range(8):
            (units, shortname) = get_info(adc,ch)
            value = convert(adc,ch,1240)
            print(f"{shortname} = {value} [{units}]")